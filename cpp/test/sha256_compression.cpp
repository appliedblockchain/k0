...
string get_python_hash(string input_hex) {
    ...
    if (remove_result != 0) {
        string reason = (string)"Could not delete temporary file with Python " +
...
TEST(SHA256Compression, CorrectnessOfPythonImplementation) {
    ...
    for (auto ex : examples) {
        string output_python_hex = get_python_hash(ex.input_hex);
...
TEST(SHA256Compression, EqualityOfImplementations) {
    for (int i = 0; i < 1; i++) {
        ...
        sha256_compression_function_gadget<FieldT> hasher(pb, IV, block, result, "hasher");
