...
vector<bool> hex2bits(const string prefixedinput) {
    if (prefixedinput.length() == 0) {
        throw invalid_argument("input string is empty");
    }
    if (strcmp(prefixedinput.substr(0,2).c_str(), "0x") != 0) {
        throw std::invalid_argument("input does not start with 0x");
    }
    auto input = prefixedinput.substr(2);
    if (!(input.length() % 8 == 0)) {
        throw invalid_argument(
            "input hex string (" + input + ") is of length " +
            to_string(input.length()) + " (not a multiple of 8)");
    }
...
std::vector<std::vector<std::vector<bool>>> hex2bitsVecVec(
...
std::string bits2hex(std::vector<bool> bv) {
    ...
    for (uint i = 0; i < bv.size(); i++) {
        ...
        if (i % 32 == 32 - 1 || i == bv.size() - 1) {
            str_stream << std::setfill('0') << std::setw(8) << std::hex;
            str_stream << word.to_ulong();
        }
    }
