...
#define ZKTRADE_DUMMYHASH_GADGET_TCC
    ...
    zktrade::dummyhash_compression_function_gadget<FieldT>::dummyhash_compression_function_gadget(protoboard<FieldT>
    &pb,
    ...
    output(output) {
        ...
        message_schedule.reset(
                new sha256_message_schedule_gadget<FieldT>(pb, new_block,
                                                           packed_W,
        ...
        for (size_t i = 0; i < 16; ++i) {
            ...
            new_round_a_variables.allocate(pb, 32, FMT(this->annotation_prefix,
                                                       " new_round_a_variables_%zu",
                                                       i + 1));
            round_a.emplace_back(new_round_a_variables);

            pb_variable_array <FieldT> new_round_e_variables;
            new_round_e_variables.allocate(pb, 32, FMT(this->annotation_prefix,
                                                       " new_round_e_variables_%zu",
                                                       i + 1));
        ...
        /* finalize */
        unreduced_output.allocate(pb, 8, FMT(this->annotation_prefix,
                                             " unreduced_output"));
        reduced_output.allocate(pb, 8, FMT(this->annotation_prefix,
                                           " reduced_output"));
    ...
    zktrade::dummyhash_compression_function_gadget<FieldT>::generate_r1cs_witness() {
        ...
        for (size_t i = 0; i < 4; ++i) {
            this->pb.val(unreduced_output[i]) =
                    ...
                    this->pb.val(round_functions[15 - i].packed_new_a);
            this->pb.val(unreduced_output[4 + i]) =
                    this->pb.val(round_functions[3 - i].packed_h) +
                    this->pb.val(round_functions[15 - i].packed_new_e);
        }
...
template<typename FieldT>
zktrade::dummyhash_two_to_one_hash_gadget<FieldT>::dummyhash_two_to_one_hash_gadget(protoboard<FieldT> &pb,
    ...
    gadget<FieldT>(pb, annotation_prefix)
{
    ...
    f.reset(new dummyhash_compression_function_gadget<FieldT>(pb, SHA256_default_IV<FieldT>(pb), block, output, FMT(this->annotation_prefix, " f")));
}
    ...
    gadget<FieldT>(pb, annotation_prefix)
{
    ...
    f.reset(new zktrade::dummyhash_compression_function_gadget<FieldT>(pb, SHA256_default_IV<FieldT>(pb), input_block.bits, output, FMT(this->annotation_prefix, " f")));
}

template<typename FieldT>
void zktrade::dummyhash_two_to_one_hash_gadget<FieldT>::generate_r1cs_constraints(const bool ensure_output_bitness)
{
...
template<typename FieldT>
void zktrade::dummyhash_two_to_one_hash_gadget<FieldT>::generate_r1cs_witness()
{
...
template<typename FieldT>
libff::bit_vector zktrade::dummyhash_two_to_one_hash_gadget<FieldT>::get_hash(const libff::bit_vector &input)
{
    ...
    dummyhash_two_to_one_hash_gadget<FieldT> f(pb, SHA256_block_size, input_variable, output_variable, "f");
...
template<typename FieldT>
size_t zktrade::dummyhash_two_to_one_hash_gadget<FieldT>::expected_constraints(const bool ensure_output_bitness)
{
    libff::UNUSED(ensure_output_bitness);
...
template<typename FieldT>
libff::bit_vector zktrade::dummyhash_compression_gadget<FieldT>::get_hash(const libff::bit_vector &input) {
    protoboard<FieldT> pb;
