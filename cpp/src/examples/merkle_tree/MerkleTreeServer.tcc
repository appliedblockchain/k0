...
MerkleTreeServer<FieldT, HashT>::MerkleTreeServer(size_t height, AbstractServerConnector &connector, serverVersion_t type) : 
    MerkleTreeStubServer(connector, type),
    tree_height{height},
    mt{height},
...
template<typename FieldT, typename HashT>
Json::Value MerkleTreeServer<FieldT, HashT>::proveInclusion(int address) {
  if (mt.num_elements() == 0 || address > (mt.num_elements() - 1)) {
    throw JsonRpcException(-32602, "Address too big");
  }

  libff::bit_vector leaf_bv = mt[address];
  ...
  inclusion_circuit.leaf_digest->generate_r1cs_witness(leaf_bv);
